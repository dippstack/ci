...
deny "docs(readme): обновить README" "префикс типа"
deny "ci!: сломать всё" "префикс типа"
pass "ci: pr-title стоит во всех четырёх рецептах"
pass "docs: канон формы описывает правила отправки"
pass "test: гарнитура отката ловит якорь извлечения"
...
deny "" "пустой"
deny "deliver: $(printf 'слово %.0s' $(seq 1 30))" "160 символов"
pass "Камера снова открывается на iOS 17 при добавлении фото в заявку и не гаснет, когда пользователь листает список и возвращается"
LC_ALL=C bash "$HERE/check.sh" "Камера снова открывается на iOS 17 при добавлении фото в заявку и не гаснет, когда пользователь листает список и возвращается назад" >/dev/null && ok "длина — символами, не байтами (LC_ALL=C)" || bad "под LC_ALL=C русский заголовок отвергнут по байтам"
