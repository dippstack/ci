...
#      (от модели ответ ждём строками «ПРИЧИНА: …» и «ПОХОЖИЙ: <slug>»; всё без маркера отбрасывается)
...
# Системный промпт ЗАМЕНЯЕТ штатный (--system-prompt, не --append): со штатным Sonnet в трети
# прогонов «шёл проверять память» или печатал маркер инструмента вместо ответа (слайс-проверка
# 23.09, 3 из 8). Ответ принимается только строкой с маркером ПРИЧИНА: — всё остальное мимо.
SYS="Ты — не агент и не ассистент с инструментами: у тебя нет памяти, файлов и команд, только
текст ниже. Ты пишешь для дежурного разработчика в Телеграм, почему выкатка не прошла.
Ответ — ровно одна строка вида «ПРИЧИНА: <одно предложение по-русски>»: что случилось, с
заглавной буквы, без разметки, без вводных слов, без догадок сверх лога. Назови конкретную строку лога, если она
объясняет причину. Ничего до и после этой строки не пиши.
...
строкой ровно «ПОХОЖИЙ: <slug страницы>». Иначе второй строки нет."
if [ -f "$SKILL" ]; then
  SYS="$SYS
...
RAW="$(printf '%s' "$PROMPT" | timeout "$TIMEOUT" claude -p --model "$MODEL" --tools "" --strict-mcp-config \
        --max-turns 1 --no-session-persistence --output-format text --system-prompt "$SYS" 2>"$ERR" \
...
# Предложение — только строка с маркером: преамбулы, «проверю память», маркеры инструментов — мимо.
SENTENCE="$(printf '%s\n' "$RAW" | sed -n 's/^[[:space:]]*\**ПРИЧИНА:\**[[:space:]]*//p' | head -n 1 | sed -E 's/\*\*//g')"
...
SLUG="$(printf '%s\n' "$RAW" | sed -n 's/^[[:space:]]*\**ПОХОЖИЙ:\**[[:space:]]*//p' | head -n 1 | tr -d '`* ')"
