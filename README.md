...
- **`deliver.yml`** — шлюз выкатки. `driver: kamal` — образ → реестр → `kamal deploy` с
  ...
  пусто — молчим. Сообщение в devsupport из трёх строк: что происходит («выкатываю» /
  «выкачено» / «не докатилось: причина» словами — сборка образа, kamal deploy, гейт/флап-гард,
  таймаут health), заголовок коммита ссылкой (на PR, если это squash-мерж с `(#N)` в конце,
  иначе на сам коммит) и строка «сборка · короткий SHA · окружение» со ссылкой на прогон —
  видно, куда смотреть, без похода в Actions. Шлюз пишет «выкатываю» до, правит то же
  сообщение на итог после; токен и чат из `vault-env` на раннере, нет файла — предупреждение
  и прогон молча. Раннер — вход `runner`.
