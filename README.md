...
- **`deliver.yml`** — шлюз выкатки. `driver: kamal` — образ → реестр → `kamal deploy` с
  ...
  пусто — молчим. Сообщение в devsupport — форма-чип, как у PR в Claude Code: первая строка —
  статус (что происходит — «выкатываю» / «выкачено» / «не докатилось»), счётчик изменённых
  строк (`+A −D`, посчитан diff'ом против родителя коммита) и ссылка «сборка» на прогон;
  SHA в строке нет — человеку он ничего не говорит, коммит виден в названии; окружение
  добавляется в конец строки, только если оно не дефолтное `dev`. Вторая строка —
  заголовок коммита ссылкой (на PR, если это squash-мерж с `(#N)` в конце, иначе на сам
  коммит). При ошибке третья строка — причина одним предложением (сборка образа упала, kamal
  deploy упал, гейт/флап-гард на хосте красный, health не позеленел за таймаут). Шлюз пишет
  «выкатываю» до, правит то же сообщение на итог после; токен и чат из `vault-env` на раннере,
