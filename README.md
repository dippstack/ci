...
- **`deliver.yml`** — шлюз выкатки. `driver: kamal` — образ → реестр → `kamal deploy` с
  ...
  статус (что происходит — «выкатываю» / «выкачено» / «не докатилось»), короткий SHA ссылкой
  на прогон и счётчик изменённых строк (`+A −D`, посчитан diff'ом против родителя коммита;
  окружение добавляется в конец строки, только если оно не дефолтное `dev`); вторая строка —
